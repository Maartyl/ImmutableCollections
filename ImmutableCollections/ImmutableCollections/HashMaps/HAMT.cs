...
namespace ImmutableCollections.HashMaps {
  ...
  public struct Test {
    public static void Run() {
      ...
      const int offset = 939999; //9999999 works fine again... ;; 999999 many broken
      // 939999 -- only 2 swapped;; other is: 940031
      //11100101011111011111
      //11100101011111111111

      for(int i = (offset + 0); i < (offset + 1000000); i++)
        h = h.Set(i, i);

      for(int i = (offset + 0); i < (offset + 500000); i++) {
        h = h.Set(i, -i);
      }
      ...
      for(int i = (offset + 0); i < (offset + 1000000); i++) {
        ...
        if((i < (offset + 500000) && v != -i) || (i >= (offset + 500000) && v != i))
          Console.WriteLine("Error: {0} returned {1}", i, v);
      }

      for(int i = (offset + 1000001); i < (offset + 2000000); i++) {
        int v;
        if(h.TryGetValue(i, out v))
          Console.WriteLine("Error: {0} shouldn't be present", i);
      }
